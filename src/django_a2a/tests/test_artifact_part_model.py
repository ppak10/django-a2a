...
def test_part_clean_text_type_valid():
    ...
    Part.objects.create(type=Part.PartType.TEXT, text="Some text", artifact=artifact)
...
def test_part_clean_text_type_invalid_fields():
    ...
    # Missing text field
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.TEXT, artifact=artifact)
    assert "Text field is required when type is 'text'." in str(excinfo.value)

    # text + data populated
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.TEXT, text="valid", data={"key": "val"}, artifact=artifact)
    ...
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.TEXT, text="valid", file=file_content, artifact=artifact)
    assert "Only `text` should be populated for type 'text'." in str(excinfo.value)
...
def test_part_clean_data_type_valid():
    ...
    Part.objects.create(type=Part.PartType.DATA, data={"key": "value"}, artifact=artifact)
...
def test_part_clean_data_type_invalid_fields():
    ...
    # Missing data field
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.DATA, artifact=artifact)
    assert "Data field is required when type is 'data'." in str(excinfo.value)

    # data + text populated
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.DATA, data={"key": "value"}, text="text", artifact=artifact)
    assert "Only `data` should be populated for type 'data'." in str(excinfo.value)

    # data + file populated
    file_content = FileContent.objects.create(name="file.txt")
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.DATA, data={"key": "value"}, file=file_content, artifact=artifact)
    assert "Only `data` should be populated for type 'data'." in str(excinfo.value)
...
def test_part_clean_file_type_valid():
    ...
    file_content = FileContent.objects.create(name="file.txt")
    Part.objects.create(type=Part.PartType.FILE, file=file_content, artifact=artifact)
...
def test_part_clean_file_type_invalid_fields():
    ...
    # Missing file field
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.FILE, artifact=artifact)
    assert "File field is required when type is 'file'." in str(excinfo.value)

    # file + text populated
    file_content = FileContent.objects.create(name="file.txt")
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.FILE, file=file_content, text="text", artifact=artifact)
    assert "Only `file` should be populated for type 'file'." in str(excinfo.value)

    # file + data populated
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type=Part.PartType.FILE, file=file_content, data={"key": "val"}, artifact=artifact)
    assert "Only `file` should be populated for type 'file'." in str(excinfo.value)
...
def test_part_clean_invalid_type():
    ...
    with pytest.raises(ValidationError) as excinfo:
        Part.objects.create(type="invalid_type", artifact=artifact)
...
def test_create_valid_part_instances():
    ...
    # Text part
    Part.objects.create(type=Part.PartType.TEXT, text="hello", artifact=artifact)

    # Data part
    Part.objects.create(type=Part.PartType.DATA, data={"foo": "bar"}, artifact=artifact)
