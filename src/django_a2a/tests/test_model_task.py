...
from django_a2a.models import Message, Part, Task, TaskStatus
from django.utils.timezone import now


class TaskModelTest(TestCase):
    def test_create_task(self):
        task = Task.objects.create(contextId=uuid4(), metadata={"key": "value"})
    ...
    def test_task_allow_non_unique_contextId(self):
        context_id = uuid4()

        Task.objects.create(contextId=context_id)
        Task.objects.create(contextId=context_id)
...
class TaskStatusModelTest(TestCase):
    def setUp(self):
        ...
        self.part = Part.objects.create(kind="text", text="Hello!", message=self.message)
